...
- Page break and line break now open a small prompt (built on the existing `'simple'` dialog type, not a new one) asking for a page/line number before inserting - cancelling inserts nothing, and leaving the field blank inserts a bare `<pb/>`/`<lb/>` with no `n` attribute.
