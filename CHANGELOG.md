...
- Fixed a `seq` collision the out-of-band D1 seed script (`generate-entity-sync-seed.mjs`) could hand out during its own documented usage: seeding a large authority file is meant to be spread across several `wrangler d1 import` runs against D1's free-tier write cap, one file per day if needed, and a real device can push a genuine edit at any point in that multi-day window. The script only wrote the `sync_counter` row (the pull-cursor high-water mark new pushes are reserved against) in its *last* file — so a push landing after only the first file or two had been imported would be reserved a low `seq` the script had already hardcoded into a not-yet-imported later file. `seq` carries no uniqueness constraint server-side, so the collision wouldn't error; it would silently strand whichever of the two rows a client's cursor doesn't land on, violating the sync protocol's own "seq monotonic and unique per owner" invariant. The counter now reserves the whole range in the first file, before any row lands.
