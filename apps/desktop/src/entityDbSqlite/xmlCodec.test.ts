...
describe('entity XML codec', () => {
  it('imports the legacy entity database and exports a re-importable database', async () => {
    ...
    expect((await first.getSummary('person-40f8324a-1498-4a87-aa27-f4025a9f2e99'))?.names[0]?.text).toBe(
      '江祏',
    );
    expect((await first.getSummary('work-828e5bea-eecd-4e0b-8fe1-b07b137041bc'))?.names[0]?.text).toBe(
      '南齊書',
    );
  ...
  it('structures date ranges, name roles, office data, relations, and extension data', async () => {
    ...
    expect(
      (await repository.backend.get("SELECT name_role FROM entity_names WHERE entity_id = ? AND name_role = 'family'", ['person-a'])),
    ).toEqual(expect.objectContaining({ name_role: 'family' }));
    expect(
      (await repository.backend.get('SELECT family_name FROM people WHERE entity_id = ?', ['person-a'])),
    ).toEqual({ family_name: '甲氏' });
    expect(
      (await repository.backend.get('SELECT from_circa, not_after, date_system, calendar_payload FROM entity_dates WHERE entity_id = ?', ['person-a'])),
    ).toEqual(
      expect.objectContaining({ from_circa: 1, not_after: '0503', date_system: 'sanmiao' }),
    );
    expect(
      (await repository.backend.get('SELECT COUNT(*) AS count FROM person_offices WHERE person_id = ?', ['person-a'])),
    ).toEqual({ count: 1 });
    expect(
      (await repository.backend.get('SELECT COUNT(*) AS count FROM office_classifications WHERE office_id = ?', ['office-a'])),
    ).toEqual({ count: 1 });
  ...
  it('extracts and backfills decision target refs from sibling XML', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_decisions
         ...
         VALUES (?, ?, ?, ?, ?)`, ['person-a', 'duplicate-ok', null, 'xml', '2026-01-01']));
    const report = (await backfillDecisionTargetsFromXml(repository, xml));
    expect(report).toEqual({ updated: 1, inserted: 1, unchanged: 0 });
    expect(
      (await repository.backend.get(`SELECT target_refs FROM entity_decisions
           WHERE entity_id = ? AND decision_type = 'duplicate-ok'`, ['person-a'])),
  ...
  it('round-trips a "thing" entity and a relation through export/import', async () => {
    ...
    expect(
      (await reimported.backend.get('SELECT relation_type, subject_entity_id, object_entity_id FROM entity_relations')),
    ).toEqual({
