...
describe('EntitySqliteRepository', () => {
  ...
  it('stores and replaces rich entity note fragments without touching descriptions', async () => {
    const repository = await EntitySqliteRepository.open();
    (await repository.createEntity({ id: 'person-note-1', kind: 'person', description: 'Short bio' }));
    const first =
  ...
  it('searches active names without scanning the XML document', async () => {
    const repository = await EntitySqliteRepository.open();
    (await repository.createEntity({ id: 'person-search-1', kind: 'person', description: 'A person' }));
  ...
  it('updates one name transactionally and preserves a tombstone', async () => {
    ...
    expect(
      (await repository.backend.get('SELECT reason FROM entity_tombstones WHERE table_name = ? AND row_id = ?', ['entity_names', name.id])),
  ...
  it('updates and tombstones all active assertions for a displayed name', async () => {
    ...
    expect((await repository.listNames('person-name-ops'))).toEqual([]);
    expect(
      (await repository.backend.get("SELECT COUNT(*) AS count FROM entity_tombstones WHERE entity_id = ? AND table_name = 'entity_names'", ['person-name-ops'])),
    ).toEqual({ count: 2 });
    (await repository.close());
  });
  ...
  it('returns a panel snapshot with fields and provenance-bearing assertions', async () => {
    const repository = await EntitySqliteRepository.open();
    (await repository.createEntity({ id: 'person-panel-1', kind: 'person', description: 'A scholar' }));
    (await repository.addName({ entityId: 'person-panel-1', text: '孔遺', isPrimary: true }));
    (await repository.backend.run(`INSERT INTO entity_authorities
          (entity_id, authority_type, authority_value, origin, source, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, ['person-panel-1',
        'Wikidata',
        'Q1',
        'authority',
        'Wikidata',
        ...
        '2026-01-01',
        '2026-01-01',]));
    (await repository.backend.run(`INSERT INTO entity_dates
  ...
  it('reads the database UUID from SQLite metadata without XML export', async () => {
    const repository = await EntitySqliteRepository.open();
    (await repository.backend.run('INSERT INTO database_metadata (key, value) VALUES (?, ?)', ['database_id', 'db-test-uuid']));
  ...
  it('includes noble titles and work authors in the panel snapshot', async () => {
    ...
    (await repository.backend.run(`INSERT INTO work_authors
          (work_id, label, reference, origin, source, status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, ['work-author-1',
        '孔遺',
        'person-title-1',
        'authority',
        'CBDB',
  ...
  it('detects duplicate active authority identifiers from SQLite', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_decisions
          (entity_id, decision_type, target_refs, origin, created_at)
         VALUES (?, ?, ?, ?, ?)`, ['person-dup-1', 'duplicate-ok', '#person-dup-1 #person-dup-2', 'user', '2026-01-01']));
    expect((await repository.listAuthorityDuplicates())).toEqual([]);
    (await repository.close());
  });
  ...
  it('marks intentional duplicate groups and backfills missing target_refs from XML notes', async () => {
    ...
    (await broken.backend.run(`INSERT INTO entity_decisions
          (entity_id, decision_type, target_refs, origin, created_at)
         VALUES (?, ?, ?, ?, ?)`, ['person-old-1', 'duplicate-ok', null, 'xml', '2026-01-01']));
  ...
  it('soft-deletes entities and merges names, authorities, and central mappings', async () => {
    ...
    expect((await repository.getPanelSummary('person-keep'))?.names.map((name) => name.text)).toEqual(
  ...
  it('finds entities by authority and by unique name+dates for promotion', async () => {
    ...
    expect((await repository.findAllEntityIdsByAuthority('person', 'CBDB', '42'))).toEqual(['person-auth']);
    expect((await repository.findAllEntityIdsByAuthority('place', 'CBDB', '42'))).toEqual([]);
    expect((await repository.findEntityIdByNameDates('person', '孔遺', 479, null))).toBe('person-auth');
  ...
  it('mutates description, dates, nationality, origin, title, authors, and authorities in transactions', async () => {
    ...
    (await repository.backend.run(`UPDATE entity_authorities SET origin = 'authority', source = 'Wikidata' WHERE entity_id = ?`, ['person-mut-1']));
    ...
    expect(
      (await repository.decoupleAuthority({ entityId: 'person-mut-1', type: 'Wikidata', value: 'Q42' })),
  ...
  it('mutates family and given names into people scalars and entity_names', async () => {
    ...
    expect(
      (await repository.backend.get('SELECT family_name, given_name FROM people WHERE entity_id = ?', ['person-fg-1'])),
    ...
    expect(
      (await repository.backend.get('SELECT family_name FROM people WHERE entity_id = ?', ['person-fg-1'])),
  ...
  it('inserts a typed name when classifying a mention surface not yet on the entity', async () => {
    ...
    expect(
      (await repository.backend.get(`SELECT name_type, name_role FROM entity_names
           WHERE entity_id = ? AND text = ? AND status = 'active'`, ['person-zi-1', '圖寧'])),
  ...
  it('includes office affiliations as role assertions in the panel snapshot', async () => {
    ...
    (await repository.backend.run(`INSERT INTO person_offices
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`, ['person-office-1', '舊職', 'authority', 'DILA', 'rejected', '2026-01-01', '2026-01-01']));
  ...
  it('applies, conflicts, and rejects CBDB concordance associations', async () => {
    ...
    expect((await repository.getPanelSummary('person-conc-1'))?.rejectedConcordances).toHaveLength(1);
    ...
    expect((await conflictRepo.applyConcordanceAssociations([association])).conflicts).toHaveLength(1);
  ...
  it('applies authority backfill patches without resurrecting rejected values', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_names
           (entity_id, text, name_type, name_role, language, is_primary, origin, source, status, created_at, updated_at)
         VALUES (?, '世遠', 'courtesy', 'variant', NULL, 0, 'authority', 'CBDB', 'rejected', ?, ?)`, ['person-bf', '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z']));
  ...
  it('keeps all family variants but sets canonical 姓 from the preferred patch field', async () => {
    ...
    (await repository.backend.run('UPDATE people SET family_name = ? WHERE entity_id = ?', ['元', 'person-tuoba']));
  ...
  it('restores withdrawn family/given names and clears non-fine authority vitals', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_names
           (entity_id, text, name_type, name_role, language, is_primary, origin, source, status, created_at, updated_at)
         VALUES (?, '陳', 'family', 'family', NULL, 0, 'authority', 'CBDB', 'withdrawn', ?, ?)`, ['person-restore', '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z']));
    (await repository.backend.run(`INSERT INTO entity_names
           (entity_id, text, name_type, name_role, language, is_primary, origin, source, status, created_at, updated_at)
         VALUES (?, '顯達', 'given', 'given', NULL, 0, 'authority', 'CBDB', 'withdrawn', ?, ?)`, ['person-restore', '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z']));
    const familyId = (
      (await repository.backend.get(`SELECT id FROM entity_names WHERE entity_id = ? AND text = '陳'`, ['person-restore'])) as { id: number }
    ).id;
    (await repository.backend.run(`INSERT INTO entity_tombstones (entity_id, table_name, row_id, reason, created_at)
         VALUES (?, 'entity_names', ?, 'mirror-copy-entity_names-status', ?)`, ['person-restore', familyId, '2026-01-01T00:00:00.000Z']));
    (await repository.backend.run(`INSERT INTO entity_dates
         ...
         VALUES (?, 'birth', 479, '0479', 'authority', 'CBDB', 'active', ?, ?),
                ...
                (?, 'death', 500, '0500', 'authority', 'DILA', 'active', ?, ?)`, ['person-restore',
        ...
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z',]));
    (await repository.backend.run('UPDATE people SET family_name = ?, given_name = ? WHERE entity_id = ?', ['陳', '顯達', 'person-restore']));
  ...
  it('stores floruit as dates+fl. and clears wrongly minted birth/death for that source', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_dates
           (entity_id, date_kind, start_year, when_value, origin, source, status, created_at, updated_at)
         VALUES (?, 'birth', 479, '0479', 'authority', 'CBDB', 'active', ?, ?),
                (?, 'death', 502, '0502', 'authority', 'CBDB', 'active', ?, ?)`, ['person-floruit',
        '2026-01-01T00:00:00.000Z',
        '2026-01-01T00:00:00.000Z',
        'person-floruit',
        '2026-01-01T00:00:00.000Z',
    ...
    const vitalKinds = (
      (await repository.backend.all(`SELECT date_kind FROM entity_dates
           WHERE entity_id = ? AND origin = 'authority' AND status = 'active'`, ['person-floruit'])) as { date_kind: string }[]
  ...
  it('rewrites unvalidated invented 姓名 on re-backfill and clears missing given', async () => {
    ...
    (await repository.backend.run('UPDATE people SET family_name = ?, given_name = ? WHERE entity_id = ?', ['孝', '武昭路太后', 'person-empress']));
  ...
  it('rolls back a failed transaction', async () => {
    ...
    await expect(
      repository.transaction(async () => {
        await repository.backend.run(`INSERT INTO entities (id, kind, created_at, updated_at)
             VALUES (?, ?, ?, ?)`, ['nested-not-supported', 'place', '2026-01-01', '2026-01-01']);
  ...
  it('autoCleanNames promotes Latn, dedupes typed names, and removes untyped', async () => {
    ...
    (await repository.backend.run(`UPDATE entity_names SET name_type = NULL
         WHERE entity_id = ? AND language LIKE '%-Latn'`, ['person-auto-clean']));
    (await repository.addName({
      entityId: 'person-auto-clean',
      text: 'orphan-untyped',
      origin: 'user',
    ...
    (await repository.backend.run(`INSERT INTO entity_names
         ...
         FROM entity_names WHERE entity_id = ? AND text = ? LIMIT 1`, ['person-auto-clean', '摩詰']));
  ...
  it('setRomanizedName stores name_type romanization with a Latn language', async () => {
    ...
    const latn = (await repository.listNames('place-rom')).find((n) => n.language?.includes('Latn'));
    expect(latn).toEqual(
      expect.objectContaining({
        text: 'Jiankang',
  ...
  it('migration 8 retags legacy Latn translations and mis-tagged zh-Hant Latin text', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_names
         ...
         VALUES
           ...
           ('place-legacy-mistag', 'Jiang Nan', 'translation', 'variant', 'zh-Hant', 0, 'user', NULL, 'active', ?, ?)`, [now, now, now, now, now, now, now, now]));
  ...
  it('addName with nameType translation stores in entity_translations', async () => {
    ...
    expect((await repository.listNames('work-gloss')).some((n) => n.text === 'Livre des Jin')).toBe(false);
  ...
  it('migration 9 moves vernacular glosses from entity_names into entity_translations', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_names
         ...
         VALUES
           ...
           ('work-legacy-gloss', 'Livre des Jin', 'translation', 'variant', 'fr', 0, 'user', NULL, 'active', ?, ?)`, [now, now, now, now]));

    await repository.backend.exec('PRAGMA user_version = 8');
    await applyEntityDbMigrations(repository.backend);

    expect((await repository.listNames('work-legacy-gloss')).some((n) => n.text === 'Livre des Jin')).toBe(
      false,
    );
  ...
  it('migration 10 withdraws dynasty spans stored as birth/death, keeps real lifespans', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_dates
         ...
         VALUES
           ...
           ('person-ambiguous-multi', 'death', 907, 'authority', 'CBDB', 'active', ?, ?)`, [...Array.from({ length: 22 }, () => now)]));
  ...
  it('normalizes mechanical name artifacts during ordinary name writes', async () => {
    ...
    (await repository.addName({ entityId: 'person-name-integrity', text: '摩詰', nameType: 'courtesy' }));
    (await repository.addName({ entityId: 'person-name-integrity', text: '摩詰', nameType: 'courtesy' }));
    (await repository.addName({ entityId: 'person-name-integrity', text: 'nan', nameType: 'variant' }));
    ...
    expect(
      (await repository.backend.get('SELECT family_name AS familyName, given_name AS givenName FROM people WHERE entity_id = ?', ['person-name-integrity'])),
  ...
  it('falls back to a generic dates row for a non-work, non-birth/death kind (place/org/office)', async () => {
    ...
    (await repository.backend.run(`INSERT INTO entity_dates
         ...
         VALUES (?, 'dates', ?, ?, 'user', NULL, 'active', ?, ?)`, ['org-1', 738, 907, '2026-01-01', '2026-01-01']));
  ...
  it('prefers a birth/death row over a generic dates row when both exist', async () => {
    ...
    (await repository.setUserEntityDate({ entityId: 'person-mixed-dates', part: 'birth', year: 400 }));
    (await repository.backend.run(`INSERT INTO entity_dates
          (entity_id, date_kind, start_year, end_year, origin, source, status, created_at, updated_at)
         VALUES (?, 'dates', ?, ?, 'user', NULL, 'active', ?, ?)`, ['person-mixed-dates', 1, 999, '2026-01-01', '2026-01-01']));
...
describe('the "thing" entity kind', () => {
  ...
  it('updateSubtype writes, overwrites, and clears a subtype', async () => {
    ...
    expect((await repository.getPanelSummary('thing-subtype-1'))?.subtype).toBe('philosophical_concept');
    expect(
      (await repository.backend.get(`SELECT value FROM entity_metadata WHERE entity_id = ? AND key = 'subtype'`, ['thing-subtype-1'])),
    ...
    expect(
      (await repository.backend.get(`SELECT COUNT(*) AS count FROM entity_metadata WHERE key = 'subtype'`)),
    ).toEqual({ count: 1 });
...
describe('entity_relations repository methods', () => {
  ...
  it('soft-removes a relation: it disappears from listings but the row survives withdrawn', async () => {
    ...
    expect(
      (await repository.backend.get('SELECT status FROM entity_relations WHERE id = ?', [relationId])),
